// Ported from assets/donna-ui-kit/react/ChannelMembersTab.jsx — two invite
// paths (add-from-workspace + invite-by-email) and the channel roster.
import { useState } from "react";

import Icon from "../../kit/Icon";
import type { ChannelRole, KitCandidate, KitChannelMember } from "./types";

const ROLES: ChannelRole[] = ["admin", "member"];

export interface ChannelMembersTabProps {
        ...
        <>
          ...
          {matches.map((c) => (
            <div className="dn-row dn-row--dashed" key={c.id}>
              <span className="dn-avatar" style={{ background: c.color }}>
                {c.initials}
              </span>
              <div>
                <div className="dn-name">{c.name}</div>
                <div className="dn-meta">
                  {c.email} · in workspace, not in this channel
                </div>
              </div>
              <button
          ...
          <form
            className="dn-row dn-row--dashed"
            onSubmit={(e) => {
              e.preventDefault();
              onInviteByEmail?.(new FormData(e.currentTarget));
            }}
          >
            ...
            <div style={{ flex: 1 }}>
              ...
              <div className="dn-meta">
                Adds them to the workspace <b>and</b> drops them into this channel
              </div>
            </div>
            <input
              className="dn-input"
              ...
              style={{ maxWidth: 200 }}
            />
            <button className="dn-mini dn-mini--grape" type="submit">
              Invite
            </button>
          </form>
        </>
            ...
            {isChannelAdmin && !m.is_you ? (
              <>
                <select
                  ...
                  value={m.role}
                  onChange={(e) =>
                    onRoleChange?.(m.id, e.target.value as ChannelRole)
                  }
                >
                  {ROLES.map((r) => (
