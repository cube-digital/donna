// ArtifactsRail — Cowork-style panel listing channel's drafts +
...
// `artifact.updated` event routed to documents store.
...
export function ArtifactsRail({ channelId }: ArtifactsRailProps) {
  const docs = useArtifacts((s) => s.byChannel[channelId] ?? []);
  const load = useArtifacts((s) => s.load);
  const loading = useArtifacts((s) => s.loading[channelId]);
  const [selected, setSelected] = useState<ChannelArtifact | null>(null);
